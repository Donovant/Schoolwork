...
int main()
{
	...
	for(int x = 0; x < row; x++)  // If first letter of the word is lowercase it converts to uppercase.
	{
		if((*words[x] >= 97) && (*words[x] <= 122))
			words[x][0] -= 32;
	}

	for(int i = 0; i < row - 1; i++)  // sorting the words in ascending order.
	{
		char temp[255];
		int min = i;
	...
	for(int x = 0; x < row; x++) // Output of the words array after sort.
	{
		if((*words[x] >= 97) && (*words[x] <= 122))
			words[x][0] -= 32;
		cout << words[x] << endl;
